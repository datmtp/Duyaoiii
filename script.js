...
function init() {
    ...
    let nihahahaSoundSrc = './sounds/qq.mp3';
    let uwahSoundSrc = './sounds/aa.mp3';
    let isBgmOn = false;
    bgm.volume = 0.8;
    ...
    function handleBodyClick(event) {
        ...
        const iconType = Math.random() < 0.9 ? 'dau' : 'day';
        playSound(iconType);
        addIcon(event, iconType);
    }

    function playSound(type) {
        const sound = new Audio(type === 'dau' ? nihahahaSoundSrc : uwahSoundSrc);
        sound.play();
    }
